...
hand_gesture_model = YOLO("/path/to/your/handgestures/model.pt")

# Load Keras model for facial expressions
expression_model = tf.keras.models.load_model("/path/to/your/facialexpressions/model.keras")

# Load OpenCV's pre-trained face detector (Haar Cascade)
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")

# Define class labels for 7 expressions (modify as needed with different datasets)
class_labels = ["Angry", "Disgust", "Fear", "Happy", "Neutral", "Sad", "Surprise"]

# Function for predicting object detection. Raise confidence as model gets better
...
def predict_expression(chosen_model, face_img):
    ...
    img_gray = cv2.cvtColor(face_img, cv2.COLOR_BGR2GRAY)  # Convert to grayscale (model is trained on grayscale images)
...
fear_timeout = 5  # Timeout in 5 seconds if fear not detected
...
while True:
    ...
    if not sequence_detected:
        ...
        for (x, y, w, h) in faces:
            ...
            # Check for "fear"
            if expression == "Fear" and confidence > 0.5:
                fear_detected = True
